...
export default function OnboardingFlow({
  ...
  const handleCardMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    card.style.setProperty("--rotateX", `${((y - centerY) / centerY) * -3}deg`);
    card.style.setProperty("--rotateY", `${((x - centerX) / centerX) * 3}deg`);
      ...
      <div className="min-h-screen px-4 md:px-8 py-8">
        ...
        >
          ...
          <div className="relative flex flex-col gap-6 mb-10">
            {/* Timeline line — orange up to active step, gray after */}
            {(() => {
              const activeIdx = completedSteps.size;
              const total = steps.length;
              // The orange section runs from top through the active step's center
              // Each step occupies 1/total of the line height
                ...
                <div key={i} style={anim("fadeSlideUp", 0.2 + i * 0.14)} className={`relative flex items-start gap-3 md:gap-5 transition-opacity duration-300 ${isLocked ? "opacity-50" : ""}`}>
                  ...
                  <div
                    className={`relative z-10 w-9 h-9 md:w-12 md:h-12 rounded-lg md:rounded-xl flex items-center justify-center font-extrabold text-sm md:text-base shrink-0 ${
                      isLocked
                        ? "bg-[var(--c-card)] text-[var(--c-border-strong)]"
                        : "bg-brand-orange text-white"
                    }`}
